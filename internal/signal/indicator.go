...
// Close near upper band → neutral (overbought-ish), near lower → neutral
// (oversold-ish), inside the body → neutral. We deliberately don't return
// strong bullish/bearish here because Bollinger touches alone don't imply
// trend direction.
func ClassifyBollinger(close, upper, _, lower float64) Status {
	...
	case close >= upper-threshold:
		return StatusBearish // overbought, mean-reversion downward bias
	case close <= lower+threshold:
		return StatusBullish // oversold, mean-reversion upward bias
	default:
		return StatusNeutral
	}
}
...
// row in the dashboard's Technical Indicators panel. The output is purely
// cosmetic; production callers may also choose to render this string in
