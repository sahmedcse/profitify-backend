...
-- Restore original batch-oriented schema.
CREATE TABLE pipeline_runs (
    id                UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_pipeline_runs_status_down ON pipeline_runs (status);
...
CREATE UNIQUE INDEX idx_pts_run_ticker_stage_down ON pipeline_ticker_stages (run_id, ticker_id, stage);
CREATE INDEX idx_pts_run_status_down ON pipeline_ticker_stages (run_id, status);
CREATE INDEX idx_pts_ticker_id_down ON pipeline_ticker_stages (ticker_id);
